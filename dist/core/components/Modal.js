...
export const Modal = ({ children, backdrop = "blur", size = "md", isOpen: isOpenProp, onOpenChange, onClose, closeOnClickOutside = true, }) => {
    ...
    return (_jsx(AnimatePresence, { children: finalIsOpen && (_jsx(motion.div, { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, onClick: handleClickOutside, className: `fixed inset-0 z-50 flex items-center justify-center p-4 ${backdropClasses[backdrop]}`, children: _jsx(motion.div, { ref: modalRef, initial: { y: 20, opacity: 0 }, animate: { y: 0, opacity: 1 }, exit: { y: 20, opacity: 0 }, transition: { type: "spring", damping: 25, stiffness: 500 }, className: `relative w-full ${sizeClasses[size]} max-h-[calc(100vh-2rem)] bg-card rounded-lg shadow-xl overflow-hidden flex flex-col`, children: React.Children.map(children, (child) => {
